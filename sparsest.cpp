...
alignas(kHardwareDestructiveInterferenceSize)
    uint24 words[std::extent_v<decltype(counts)>];
...
int main(int argc, char * argv[])
{
    ...
    if ((false)) {
        ...
        auto lowerAddress =
            reinterpret_cast<std::uintptr_t>(&*std::cbegin(counts));
        auto upperAddress =
            reinterpret_cast<std::uintptr_t>(&*std::cend(counts));
        ...
        auto isPmPresent = [](const PmEntry & entry) {
            return (entry & kPmPresent) != 0;
        };
        auto hi = std::cbegin(pagemap);
        for (;;) {
            auto lo = std::find_if(hi, std::cend(pagemap), isPmPresent);
            if (lo == std::cend(pagemap)) {
                break;
            }
            hi = std::find_if_not(lo, std::cend(pagemap), isPmPresent);
